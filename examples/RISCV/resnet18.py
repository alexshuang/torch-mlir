...
#print("load labels ...", file=sys.stderr)
#labels = load_labels()

print("load image from " + image_url, file=sys.stderr)
img = torch.randn(1, 3, 224, 224).to(dtype=torch.float32)
...
open('torch_resnet18.mlir', 'w').write(mlir)
backend = refbackend.RefBackendLinalgOnTensorsBackend()
compiled = backend.compile(module)
jit_module = backend.load(compiled)

logits = jit_module.forward(img.numpy())
print(f"results: {logits.shape}")
print(logits)
